...
<svelte:head>
  <title
    >{bean?.name ? `${bean.name} by ${bean.roaster}` : "Coffee Bean"} | Kissaten</title
  >
  <meta
    name="description"
  ...
  {#if bean}
    ...
    <meta
      ...
      content={`${bean.name} by ${bean.roaster}`}
    />
    <meta
      property="product:price:amount"
      content={bean.price?.toString() ?? ""}
    />
    ...
    {@html `<script type="application/ld+json">${safeJsonLdStringify({
      ...
      name: bean.name,
      brand: bean.roaster
        ? { "@type": "Brand", name: bean.roaster }
        : undefined,
      description:
        bean.description || `${bean.name} coffee from ${bean.roaster}`,
      image: ogImage,
      offers: bean.price
        ? {
            "@type": "Offer",
              ...
              >
                {#if isCustomBean}
                  ...
                  >
                    ...
                    <span class="font-medium text-sm uppercase tracking-widest"
                      >Custom Bean</span
                    >
                  </div>
                {:else}
          ...
          <div class="flex justify-between items-start">
            <div class="flex-1 space-y-2">
              ...
              >
                ...
                {#if isCustomBean}
                  <div
                    class="flex items-center bg-blue-500/10 px-2 py-1 rounded-full text-blue-500 text-xs transition-all animate-in duration-300 fade-in zoom-in"
                  >
              ...
              >
                ...
                {#each uniqueVarieties as variety, index (variety)}
                  <div
                    ...
                    style="display: contents;"
                  >
                    {#if index > 0}<span class="mx-0.5">/&#8203;</span>{/if}
                    <a
          ...
          <div transition:slide|global>
            ...
            >
              <CardHeader>
                ...
                <CardDescription
                  >Personal notes about this coffee. Only visible to you.</CardDescription
                >
              </CardHeader>
              <CardContent>
                <BeanNotesEditor
        ...
        >
          ...
          <CardContent>
                  ...
                  <div class="gap-4 grid grid-cols-1 md:grid-cols-2">
                    {#if origin.country}
                      <div class="flex items-center space-x-2">
                        <Globe class="mr-1 w-4 h-4 text-muted-foreground" />
                        ...
                        <span class="font-medium">
                          ...
                          >
                            {origin.country_full_name ||
                              countryNameFromCode(origin.country)}
                          </a>
                        </span>
                      </div>
                    {/if}
                    {#if origin.region}
                      <div class="flex items-center space-x-2">
                        <MapPin class="mr-1 w-4 h-4 text-muted-foreground" />
                        <span class="font-medium text-muted-foreground"
                          >Region:</span
                        >
                        <span>
                    ...
                    {#if origin.farm}
                      <div class="flex items-center space-x-2">
                        <TreePine class="mr-1 w-4 h-4 text-muted-foreground" />
                        <span class="font-medium text-muted-foreground"
                          >Farm:</span
                        >
                        <span>
                    ...
                    {#if origin.process}
                      <div class="flex items-center space-x-2">
                        <Droplets class="mr-1 w-4 h-4 text-muted-foreground" />
                        <span class="font-medium text-muted-foreground"
                          >Process:</span
                        >
                        <span
                          ><a
                            ...
                            class="hover:underline">{origin.process}</a
                          ></span
                        >
                      </div>
                    {/if}
                    {#if origin.variety}
                      <div class="flex items-center space-x-2">
                        <Leaf class="mr-1 w-4 h-4 text-muted-foreground" />
                        <span class="font-medium text-muted-foreground"
                          >Variety:</span
                        >
                        <span
                          >{#each origin.variety_canonical ?? [] as variety, i}
                            {#if i > 0},&nbsp;{/if}<a
                              href={`/varietals/${api.normalizeVarietalName(variety)}`}
                              class="hover:underline">{variety}</a
                            >
                          {/each}</span
                        >
                      </div>
                    ...
                    {#if origin.harvest_date}
                      <div class="flex items-center space-x-2">
                        <Calendar class="mr-1 w-4 h-4 text-muted-foreground" />
                        <span class="font-medium text-muted-foreground"
                          >Harvest Date:</span
                        >
                        <span
            ...
            <div class="flex items-center space-x-2 mt-4 pt-4 border-t">
              ...
              <span>{bean.is_single_origin ? "Single Origin" : "Blend"}</span>
            </div>
          </CardContent>
        </Card>
      </div>
        ...
        >
          ...
          <CardContent class="space-y-4">
            ...
            {#if bean.in_stock !== null && !isCustomBean}
              <div class="flex justify-between items-center space-x-2">
                ...
                >
                  ...
                  title={new Date(bean.scraped_at).toLocaleString("en-GB")}
                  >(checked {humanizeDuration(
                    new Date().getTime() - new Date(bean.scraped_at).getTime(),
                  )} ago)
                </span>
              </div>
            {/if}
            ...
            {#if userSettings.betaEnabled}
              <Button
                variant="outline"
                class="dark:hover:bg-cyan-500/10 mt-1 py-2 border-primary/20 dark:border-cyan-500/20 w-full h-auto dark:hover:text-white dark:text-white text-center leading-tight whitespace-normal transition-colors"
            ...
            <div class="space-y-2 text-sm">
                ...
                <div class="flex justify-between">
                  ...
                  <span
                    >{formatPrice(
                      ...
                      bean.currency_of_price_paid_for_green_coffee || "USD",
                    )}/kg</span
                  >
                </div>
              {/if}
              <div class="flex justify-between">
                ...
                >
                <span title={new Date(bean.date_added).toLocaleString("en-GB")}
                  >{humanizeDuration(
                    new Date().getTime() - new Date(bean.date_added).getTime(),
                  )} ago</span
                >
              </div>
              {#if !isCustomBean}

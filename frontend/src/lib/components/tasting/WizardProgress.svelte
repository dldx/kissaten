<script lang="ts">
	...
	let tooltipX = $derived.by(() => {
		...
		if (!trigger || !containerEl) return null;
		const tRect = trigger.getBoundingClientRect();
		const cRect = containerEl.getBoundingClientRect();
		return tRect.left - cRect.left + tRect.width / 2;
	});
...
>
	...
	<div class="relative flex justify-center items-center px-4 w-full" bind:this={containerEl}>
		...
		{#if openIndex !== null && tooltipX !== null}
			<div
				class="bottom-full z-50 absolute mb-2.5 pointer-events-none"
				style="left: {tooltipX}px; transform: translateX(-50%);"
			>
				<div class="bg-popover shadow-md px-2 py-2 border rounded-md min-w-44 text-popover-foreground">
					<ul class="flex flex-col gap-0.5">
						{#each steps as s, j}
							{@const jIsCurrent = j === current}
							{@const jIsCompleted = j < current}
							{@const jIsHovered = j === openIndex}
							<li
								class={cn(

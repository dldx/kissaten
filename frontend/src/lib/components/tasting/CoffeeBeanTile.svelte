<script lang="ts">
	...
	interface Props {
		...
		noLink?: boolean;
		/** If true, optimizes for print (removes hover, shadows, etc.) */
		variant?: "default" | "label";
		/** Optional custom note to display on labels */
		customNote?: string | null;
		/** Customizable visibility for label fields */
		showOrigin?: boolean;
		showTags?: boolean;
	...
	const originDisplay = $derived(
		bean
			? slim
				...
				: (() => {
					if (!bean.origins || bean.origins.length === 0) return 'Unknown Origin';

					const firstOrigin = bean.origins[0];
					const generateLabel = (o: any) => {
						const parts = [];
						if (o.country_full_name) parts.push(o.country_full_name);
						if (o.region) parts.push(o.region);
						if (o.farm) parts.push(o.farm);
						return parts.join(', ');
					};

					if (bean.is_single_origin) {
						return generateLabel(firstOrigin) || 'Unknown Origin';
					} else {
						// Check if all share same details
						const firstLabel = generateLabel(firstOrigin);
						const allSame = bean.origins.every(o => generateLabel(o) === firstLabel);
						if (allSame) return firstLabel;
						return bean.origins.map(generateLabel).filter(Boolean).join(' / ');
					}
				})()
			: ""
	);
...
<svelte:element
	...
	href={beanUrl}
	class={cn(
		"group relative flex items-center shadow-sm border overflow-hidden transition-all duration-200",
		isLabel
			? "bg-white border-none p-0 shadow-none h-full w-full [print-color-adjust:exact] [-webkit-print-color-adjust:exact]"
			: "bg-emerald-50/20 hover:bg-emerald-50/30 p-3 border-emerald-500/20 rounded-lg dark:hover:shadow-cyan-500/20 dark:hover:shadow-2xl dark:border-cyan-500/30 dark:bg-gradient-to-br dark:from-slate-900/80 dark:to-slate-800/80 dark:hover:border-cyan-400/60",
		beanUrl ? "cursor-pointer" : "",
		className,
	)}
>
	{#if (bean?.image_data || bean?.image_url) && !isLabel}
	...
	<div class={cn("flex flex-col flex-1 justify-center min-w-0 text-left", isLabel && "gap-0")}>
		<div class="flex flex-col mb-1 sm:mb-1.5 min-w-0">
			...
			<h3
				class={cn(
					"font-extrabold text-foreground dark:text-cyan-100 text-sm sm:text-base leading-tight",
					wrapName ? "text-wrap" : "truncate"
				)}
			>
				{bean ? (bean.name || "") : (label || "Selected Bean")}
			</h3>
		</div>
		...
		{#if bean}
			...
			{#if showTags}
				<div class="flex flex-wrap gap-1 mb-1 sm:mb-1.5 min-w-0 overflow-hidden">
					{#if bean.origins && bean.origins.length > 0}
						...
						>
							{#if !isLabel}
								...
								></iconify-icon>
							{:else}
								<MapPin class="mr-1 w-2.5 h-2.5 shrink-0" />
							{/if}
					...
					<span
						class="inline-flex items-center bg-green-100 dark:bg-emerald-900/40 px-1 sm:px-1.5 py-0.5 dark:border dark:border-emerald-400/50 rounded font-medium text-[10px] text-green-800 dark:text-emerald-200 sm:text-xs bean-tag-variety [print-color-adjust:exact] [-webkit-print-color-adjust:exact]"
					>
					...
					<span
						class="inline-flex items-center bg-blue-100 dark:bg-cyan-900/40 px-1 sm:px-1.5 py-0.5 dark:border dark:border-cyan-400/50 rounded font-medium text-[10px] text-blue-800 dark:text-cyan-200 sm:text-xs bean-tag-process [print-color-adjust:exact] [-webkit-print-color-adjust:exact]"
					>
						<Droplets
			...
			<!-- Tasting Notes -->
			{#if showTastingNotes && !slim && bean.tasting_notes && bean.tasting_notes.length > 0}
		...
		{#if customNote}
			<div class="mt-auto pt-1 font-medium text-[9px] text-muted-foreground truncate italic">
				{customNote}
			</div>
		{/if}

		{#if isLabel}
			<div class="right-1 bottom-1 absolute flex items-center gap-1">
				<div class="w-3 h-3">
					{@html Logo}
				</div>
				<span class="font-bold text-[10px] text-emerald-600 tracking-tighter">kissaten.app</span>
			</div>
		{/if}
	</div>
</svelte:element>

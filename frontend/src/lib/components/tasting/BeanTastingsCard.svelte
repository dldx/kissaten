...
>
    ...
    <CardContent class="space-y-6">
        ...
        <!-- Beta Features (History & Guided Tastings) -->
        {#if userSettings.betaEnabled}
                {#if tastings.length > 0}
                    <div class="space-y-3 mb-4">
                        <h4
                            class="flex items-center gap-2 font-semibold text-muted-foreground text-xs uppercase tracking-wider"
                        >
                            Mine
                        </h4>
                        <div class="space-y-3">
                                        ...
                                        {#if session.selectedNotes.length > 6}
                                            ...
                                            >
                                                {expandedSessions[session.id!]
                                                    ? "show less"
                                                    : `+${session.selectedNotes.length - 6}`}
                                            </button>
                                        {/if}
                                    </div>
                                </div>
                ...
                {#if isSaved}
                    {#if tastings.length > 0}
                        <Button
                            class="w-full"
                            variant="outline"
                            href="/tasting?bean={encodeURIComponent(
                                beanUrlPath,
                            )}"
                        >

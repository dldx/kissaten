<script lang="ts">
	...
	function createChartWithData(chartData: SunburstData) {
        ...
        function computeRadialGamma(depth: number) {
            ...
            return baseGamma + perLevelGamma * Math.min(depth, maxLevelsToScale);
        }
		...
		function getWeights(remaining: number): number[] {
			...
			if (weights.length < remaining) {
				const outerBoost = weights.length > 0 ? weights[weights.length - 1] : 3.0;
				weights = weights.concat(new Array(Math.max(0, remaining - weights.length - 1)).fill(1));
		...
		function mapRadius(y: number, ringR: number) {
			...
			const cum: number[] = [0];
			for (let i = 0; i < remaining; i++) cum.push(cum[i] + (weights[i] ?? 1));
			...
			const idx = Math.min(remaining - 1, Math.max(0, Math.floor(clampedY)));
		...
		const color = d3.scaleOrdinal(categories, categories.map(getFlavourCategoryHexColor));
		...
		const path = svg
			...
			.attr('fill-opacity', (d: any) => (arcVisible(d.current) ? (d.children ? 0.8 : 0.6) : 0))
            .attr('stroke', (d: any) => '#111')
            .attr('stroke-width', (d: any) => (arcVisible(d.current) ? (d.children ? 0.5 : 0.25) : 0))
		...
		function hovered(event: MouseEvent, p: any) {
			...
			path
				.attr('fill-opacity', (d: any) => {
					const base = arcVisible(d.current) ? (d.children ? 0.8 : 0.6) : 0;
					if (base === 0) return 0;
					return ancestors.includes(d) ? Math.max(base, 0.95) : Math.min(base, 0.15);
				})
				.attr('stroke', (d: any) => (ancestors.includes(d) ? '#111' : null))
				.attr('stroke-width', (d: any) => (ancestors.includes(d) ? 0.5 : null));

            // Tooltip content
            const format = d3.format(',d');
            const breadcrumb = p
                .ancestors()
                .map((d: any, index: number) => (index === 0) ? d.data.name : `<span class="font-normal">${d.data.name}</span>`)
                .reverse()
		...
		function unhovered() {
			...
			path
				.attr('fill-opacity', (d: any) => (arcVisible(d.current) ? (d.children ? 0.8 : 0.6) : 0))
				.attr('stroke', (d: any) => (arcVisible(d.current) ? (d.children ? '#111' : null) : null))
				.attr('stroke-width', (d: any) => (arcVisible(d.current) ? (d.children ? 0.5 : 0.25) : 0));
		...
		function clicked(event: MouseEvent, p: any) {
			...
			label
				...
				.attrTween(
					...
					((d: any) => {
						...
						return tween as unknown as (this: SVGTextElement, t: number) => string;
					}) as any
				);
			...
			t.on('end', () => {
				...
				path
					.attr('pointer-events', (d: any) => (arcVisible(d.current) ? 'auto' : 'none'))
					.style('cursor', (d: any) => (d.children && arcVisible(d.current) ? 'pointer' : 'default'));
...
<div class="relative {className}" style="min-height: {height}px" bind:this={containerEl}>

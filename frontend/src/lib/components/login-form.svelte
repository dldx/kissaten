	...
	{#if $session.data}
		<div class="flex flex-col items-center gap-6 text-center">
			<div class="flex flex-col items-center gap-2">
				<div
					class="flex justify-center items-center rounded-md w-12 h-12"
				>
					{@html Logo}
				</div>
		...
		<form {...form}>
			<FieldGroup>
				<div class="flex flex-col items-center gap-2 text-center">
					...
					<FieldDescription>
						Don't have an account? Just enter your email to get
						started.
					</FieldDescription>
				</div>
				...
				{#if errorMessage}
					<div
						class="bg-red-50 px-4 py-3 border border-red-200 rounded-md text-red-800"
					>
						{errorMessage}
					</div>
				{/if}

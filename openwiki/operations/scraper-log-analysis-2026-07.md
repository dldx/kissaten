...
- **Root cause** (visible in log): `ProxyError('Unable to connect to proxy', ConnectTimeoutError(..., 'Connection to REDACTED timed out. (connect timeout=10)'))` — the single shared proxy died, taking down **all scraping and Logfire exports** (`WARNING:logfire:Currently retrying N failed export(s)` ×480).

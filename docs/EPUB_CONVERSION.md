...
5. **EPUB 3 packaging** — `ebooklib` assembles the container, OPF package,
...
| Per-origin supply chain map (Farmer → … → Roaster with credit / power / gatekeeper strip; double border marks the largest non-farmgate node) | top of each origin chapter | FOB tables + Dimension matrix |
...
- **Captions carry the exact percentage ranges** plus the report's data note
  ...
  information is lost even if a reader drops images.
...
- Tables assume the report's pipe-table style; very wide tables (the
  ...
  landscape, depending on the reader.

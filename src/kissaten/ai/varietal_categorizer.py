...
Provide a clear reason based on coffee botany."""
    ...
    def dedupe_mappings_static(data: list[dict]) -> tuple[list[dict], list[dict]]:
        ...
        entries mapping to IDENTICAL ``canonical_names``) is reduced to one
        representative entry; genuine conflicts (different ``canonical_names``)
        ...
        canonical_field = "canonical_names"

        def canonical_signature(item: dict) -> object:
            return tuple(sorted(item.get(canonical_field, []) or []))

        def canonical_matches(item: dict) -> bool:
            canonicals = item.get(canonical_field, []) or []
            if len(canonicals) != 1:
                return False
            return str(item.get(name_field, "") or "").lower() == str(canonicals[0]).lower()

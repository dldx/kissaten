...
def apply_review_decisions(
    ...
    update_db: bool = typer.Option(
        False, "--update-db", help="Also write the review status directly to the rw DuckDB (data/rw_kissaten.duckdb, or KISSATEN_DATABASE_PATH if set) so approvals apply without waiting for a refresh"
    ),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Enable verbose logging"),
):
    ...
    CoffeeBeanDiffUpdate ({url, requires_review: false}) diffjson next to the bean's
    own JSON in the roaster session folder, e.g.
    data/roasters/<roaster>/<session>/<slug>_<hash8>.review.diffjson, so an
    `kissaten refresh` shows them in public search. Rejected products keep
    requires_review=true (stay hidden).
    ...
    With --update-db, approved decisions are also written directly into the rw
    DuckDB (requires_review=false), so the approval is effective before the next
    `kissaten refresh`; promotion still needs
    `cp data/rw_kissaten.duckdb data/kissaten.duckdb` + serve restart.
    """
    import re
    import sqlite3
    ...
    # Optional: write approved decisions straight into the rw DuckDB so they
    # take effect before the next `kissaten refresh`.
    db_conn = None
    db_updated_count = 0
    if update_db and not dry_run:
        # Mirror the refresh CLI: opt into the production-DB guard and the rw
        # connection before importing kissaten.api.db so its module-level conn
        # targets rw_kissaten.duckdb (or KISSATEN_DATABASE_PATH when set).
        # Save/restore env so we don't leak these overrides to the caller.
        _saved_env = {k: os.environ.get(k) for k in ("KISSATEN_ALLOW_PRODUCTION_DB", "KISSATEN_USE_RW_DB")}
        os.environ["KISSATEN_ALLOW_PRODUCTION_DB"] = "1"
        os.environ["KISSATEN_USE_RW_DB"] = "1"
        try:
            from kissaten.api.db import conn as _api_conn

            db_conn = _api_conn
            # Sanity check that the table exists; a fresh/empty DB will fail here
            db_conn.execute("SELECT COUNT(*) FROM coffee_beans").fetchone()
        except Exception as e:
            console.print(f"[yellow]Warning: could not open rw DuckDB for direct updates ({e}); "
                          "diffjson files will still be written and applied on the next refresh.[/yellow]")
            db_conn = None
        finally:
    ...
    # the derived bean_url_path and the stored product url. The derived
    # bean_url_path is indexed under BOTH the raw filename (which carries the
    # trailing _HHMMSS timestamp suffix — as stored in bean_url_path by
    # refreshes that predate timestamp-stripping) and the timestamp-stripped
    # form (as generated by the current refresh), because the frontend's stored
    # entity_url_path mirrors whichever form the DB had when the decision was
    # recorded.
    ...
    if roasters_dir.exists():
        for json_path in roasters_dir.glob("**/*.json"):
            ...
            if not url:
                continue
            # slug = filename minus .json (the trailing _HHMMSS timestamp may or
            # may not be present).
            raw_slug = json_path.name[:-5] if json_path.name.endswith(".json") else json_path.name
            stripped_slug = re.sub(r"_\d{6}$", "", raw_slug)
            roaster_dir = json_path.parent.parent.name
            entry = {"url": url, "json_path": json_path}
            bean_lookup.setdefault(f"/{roaster_dir}/{raw_slug}", entry)
            bean_lookup.setdefault(f"/{roaster_dir}/{stripped_slug}", entry)
            # For the product URL, the NEWEST session folder wins so the review
            # diff lands next to the current bean JSON (mirroring how scrapers
            # place stock/out-of-stock diffs next to the JSON). Session folder
            # names are ISO timestamps, so string comparison == chronological.
            prev = url_lookup.get(str(url))
            if prev is None or json_path.parent.name > Path(prev["json_path"]).parent.name:
                url_lookup[str(url)] = entry
    ...
    for entity_url_path, row in latest.items():
        ...
        if decision == "rejected":
            ...
            if entry is None:
                # The entity path may carry the trailing _HHMMSS timestamp (or
                # be missing it) while the lookup only has the other form.
                relaxed = re.sub(r"_\d{6}$", "", entity_url_path)
                entry = bean_lookup.get(relaxed) or url_lookup.get(relaxed)
            if entry is None:
                # No backing bean JSON -> do NOT mark applied so a later run can retry.
                action = "skipped: no bean JSON"
            else:
                product_url = entry["url"]
                target = Path(entry["json_path"]).parent / f"{_review_diffjson_filename(product_url)}.review.diffjson"
                if target.exists():
                ...
                if db_conn is not None:
                    try:
                        row = db_conn.execute(
                            "SELECT requires_review FROM coffee_beans WHERE url = ?", [product_url]
                        ).fetchone()
                        if row is None:
                            logger.warning(
                                "Bean not found in rw DuckDB (url=%s); diffjson written and will apply on next refresh", product_url
                            )
                        elif row[0] is False:
                            pass  # already public
                        else:
                            db_conn.execute(
                                "UPDATE coffee_beans SET requires_review = false WHERE url = ?", [product_url]
                            )
                            db_updated_count += 1
                    except Exception as e:
                        logger.warning("Failed to update rw DuckDB for %s: %s", product_url, e)

        table.add_row(entity_url_path, decision, action)

    # Commit any direct rw DuckDB updates made above.
    if db_conn is not None:
        try:
            db_conn.commit()
        except Exception as e:
            console.print(f"[yellow]Warning: could not commit rw DuckDB updates ({e}); "
                          "diffjson files will still be applied on the next refresh.[/yellow]")
    ...
    if update_db and not dry_run and db_conn is not None:
        summary += f"; {db_updated_count} beans updated directly in rw DuckDB"
    console.print(f"\n[dim]{summary}.[/dim]")
    ...
    if update_db and not dry_run and db_updated_count > 0:
        console.print(
            "[bold]Next:[/bold] Direct rw DuckDB updates still need "
            "`cp data/rw_kissaten.duckdb data/kissaten.duckdb` and a serve restart "
            "for the API to serve the change."
        )

"""Opal Coffee Roasters scraper implementation with AI extraction (Squarespace)."""
...
@register_scraper(
    name="opal-coffee-roasters",
    display_name="Opal Coffee Roasters",
    roaster_name="Opal Coffee Roasters",
    website="https://www.opalcoffeeroasters.co.uk",
    description=(
        "Specialty coffee roastery based in Derry/Londonderry, Northern Ireland, "
        "roasting seasonal single-origin coffees."
    ),
...
class OpalCoffeeRoastersScraper(BaseScraper):
    """Scraper for Opal Coffee Roasters (opalcoffeeroasters.co.uk) — a Squarespace storefront.
    ...
    def __init__(self, api_key: str | None = None):
        """Initialize Opal Coffee Roasters scraper."""
        super().__init__(
            roaster_name="Opal Coffee Roasters",
            base_url="https://www.opalcoffeeroasters.co.uk",
    ...
    async def get_store_urls(self) -> list[str]:
        """Return the Squarespace seasonal-coffee listing URL."""
        return ["https://www.opalcoffeeroasters.co.uk/seasonal-coffee"]
    ...
    async def _extract_product_urls_from_store(self, store_url: str) -> list[str]:
        ...
        # sold-out products with a "Sold out" badge (product-list-item-status)
        # on the card, so we skip those cards.
        """
        ...
        product_urls = []
        for item in soup.select(".product-list-item, .ProductList-item, a[href*='/seasonal-coffee/p/']"):
            ...
            if item.name == "a":
                ...
                for a in item.find_all("a", href=True):
                    if "/seasonal-coffee/p/" not in a["href"]:
                        continue
                    product_urls.append(self.resolve_url(a["href"]))

        product_urls = list(dict.fromkeys(product_urls))

        # Exclude genuine non-coffee items: the equipment collection lives on its
        # own page, but guard against gear/gift/subscription products sneaking in.
        excluded = [
            "equipment",
            "aeropress",
            "hario",
        ...
        coffee_urls = [
            ...
            if self.is_coffee_product_url(url, required_path_patterns=["/seasonal-coffee/p/"])
    ...
    async def _scrape_new_products(self, product_urls: list[str]) -> list[CoffeeBean]:
        ...
        return await self.scrape_with_ai_extraction(
            ...
            translate_to_english=False,  # Site is already in English
        )
    ...
    async def fetch_page(self, *args, **kwargs):
        ...
        if "/seasonal-coffee/p/" not in url:
            return soup

        # Opal's structured data lives in <head> meta tags. The og:description

"""New Breed Coffee scraper implementation with AI-powered extraction."""
...
@register_scraper(
    name="new-breed-coffee",
    display_name="New Breed Coffee",
    roaster_name="New Breed Coffee",
    website="https://newbreed.coffee",
    description="Specialty coffee roaster focusing on redefining Vietnamese coffee",
    requires_api_key=True,
    currency="GBP",  # British Pound
    country="United Kingdom",
    status="available",
)
class NewBreedCoffeeScraper(BaseScraper):
    """Scraper for New Breed Coffee (newbreed.coffee) with AI-powered extraction."""

    def __init__(self, api_key: str | None = None):
        """Initialize New Breed Coffee scraper.
        Args:
            api_key: Google API key for Gemini. If None, will try environment variable.
        """
        super().__init__(
            roaster_name="New Breed Coffee",
            base_url="https://newbreed.coffee",
    ...
    async def get_store_urls(self) -> list[str]:
        ...
        """
        return ["https://newbreed.coffee/collections/coffee"]
    ...
    async def fetch_page(self, *args, **kwargs) -> BeautifulSoup | Tag | None:
        ...
        try:
            ...
            product_section = soup.select("section.single-product")
    ...
    async def _extract_product_urls_from_store(self, store_url: str) -> list[str]:
        ...
        for el in all_product_url_el:
            # Check if "Sold out" class appears in the parent elements
            if "product__sold-out" not in el["class"]:
                href = el.get('href')
                if href:

"""Santu Coffee scraper implementation with AI-powered extraction.

Santu Coffee is an Edinburgh specialty coffee roaster selling on a WordPress +
WooCommerce storefront at santucoffee.com (www.santucoffee.com after the
non-www 301 redirect). The catalogue is a small set of whole-bean single-origin
Brazilians (Coffee 01/03/04 + Santu Decaf), plus subscriptions for each bean
and a Nespresso-style pod product.

Discovery uses the public unauthenticated WooCommerce Store API (product
listing + prices/variants), while per-product bean fields (origin, process,
tasting notes, weight, etc.) come from AI extraction of each product's
JSON-LD-annotated HTML page — mirroring the casa_coffee_roasters pattern.
"""
...
# catalogue (whole-bean coffees, subscriptions, and pods) with prices, variants
# and permalinks, which we filter down to whole-bean coffee below.
STORE_API_URL = "https://www.santucoffee.com/wp-json/wc/store/v1/products?per_page=100"


@register_scraper(
    name="santu",
    display_name="Santu Coffee",
    roaster_name="Santu Coffee",
    website="https://www.santucoffee.com",
    description="Edinburgh specialty coffee roaster sourcing small Brazilian lots, offering "
    "whole-bean coffees plus subscriptions and pods (WooCommerce storefront).",
    requires_api_key=True,
    currency="GBP",
    country="United Kingdom",
    status="available",
)
class SantuCoffeeScraper(BaseScraper):
    """Scraper for Santu Coffee (www.santucoffee.com) with AI-powered extraction."""

    def __init__(self, api_key: str | None = None):
        """Initialize Santu Coffee scraper.
        ...
        super().__init__(
            roaster_name="Santu Coffee",
            base_url="https://www.santucoffee.com",
    ...
    async def _extract_product_urls_from_store(self, store_url: str) -> list[str]:
        ...
        HTML class/text scanning is needed. We also drop subscriptions
        (``variable-subscription`` type) and the pod/capsule product before
        running ``is_coffee_product_url`` so excluded products can never leak
        ...
        for product in products:
            ...
            # Drop subscriptions (incl. variable-subscription) and pods/capsules.
            product_type = (product.get("type") or "").lower()
            product_name = (product.get("name") or "").lower()
            if "subscription" in product_type or "subscription" in product_name:
                logger.debug(f"Skipping subscription product: {product.get('name')}")
                continue
            if "pod" in product_name or "capsule" in product_name or "pods" in product_type:
                logger.debug(f"Skipping pod/capsule product: {product.get('name')}")
                continue

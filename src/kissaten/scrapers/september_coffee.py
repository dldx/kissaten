"""September Coffee scraper implementation with AI-powered extraction."""

import logging

from bs4 import BeautifulSoup

from ..schemas import CoffeeBean
from .registry import register_scraper
from .shopify_base import ShopifyJsonScraper

logger = logging.getLogger(__name__)


@register_scraper(
    name="september-coffee",
    display_name="September Coffee",
    roaster_name="September Coffee Company",
    website="https://september.coffee",
    description="Specialty coffee roaster based in Canada",
    requires_api_key=True,
    currency="CAD",
    country="Canada",
    status="available",
)
class SeptemberCoffeeScraper(ShopifyJsonScraper):
    """Scraper for September Coffee using Shopify products.json."""

    def __init__(self, api_key: str | None = None):
        """Initialize September Coffee scraper.

        Args:
            api_key: Google API key for Gemini. If None, will try environment variable.
        """
        super().__init__(
            roaster_name="September Coffee Company",
            base_url="https://september.coffee",
            products_json_urls=[
                "https://september.coffee/en-gb/collections/coffee/products.json",
            ],
            rate_limit_delay=2.0,  # Be respectful with rate limiting
            max_retries=3,
            timeout=30.0,
            cache_product_pages=True,  # Enable caching/screenshots
            use_optimized_mode=True,
        )

        # Exclude non-coffee products
        self.exclude_slugs = [
            "test-batch",
            "subscription",
            "gift-card",
            "gift",
            "wholesale",
            "equipment",
            "accessory",
            "merchandise",
        ]

        if api_key:
            from ..ai import CoffeeDataExtractor

            self.ai_extractor = CoffeeDataExtractor(api_key=api_key)

    def preprocess_product_url(self, url: str) -> str:
        """Standardize September Coffee URLs by removing collection segments."""
        if "/collections/" in url and "/products/" in url:
            try:
                handle = url.split("/products/")[-1].split("?")[0]
                return f"{self.base_url}/en-gb/products/{handle}"
            except Exception:
                return url
        return url

"""Smuggler's Drop scraper implementation with AI extraction.

Smuggler's Drop (smugglersdrop.co.uk) is a Squarespace-hosted specialty
coffee roaster based in Hastings, UK. The storefront (www.smugglersdrop.co.uk,
the bare domain 301-redirects to the www canonical) serves products at
``/shop/p/...`` and carries Squarespace product-page meta tags (``og:title``,
``og:description``, ``product:price:*``, ``product:availability``) plus a
``static-context`` JSON blob with per-variant size/price/currency/stock.

Discovery crawls the ``/shop`` listing ("All" collection) and keeps whole-bean
coffee products only — the core range is playing-card themed (Ace of Clubs,
King of Clubs, Queen of Hearts decaf, Jack of Clubs, The Joker). The base
exclusion patterns already drop the non-coffee items sold in the same listing:
the Ace of Spades t-shirt (``shirt``), gift cards (``gift-card``) and the
monthly gift subscriptions (``subscription``). Samplers are not excluded.
"""
...
@register_scraper(
    name="smugglers-drop",
    display_name="Smuggler's Drop",
    roaster_name="Smuggler's Drop",
    website="https://smugglersdrop.co.uk",
    description=(
        "Hastings-based specialty coffee roaster selling small-batch, "
        "playing-card-themed whole-bean coffees (Ace of Clubs, King of Clubs, "
        "Queen of Hearts decaf, Jack of Clubs and The Joker), roasted fresh "
        "from a micro roastery."
    ),
...
class SmugglersDropScraper(BaseScraper):
    """Scraper for Smuggler's Drop (smugglersdrop.co.uk) — a Squarespace storefront.
    ...
    alongside page screenshots to extract CoffeeBean models. The bare
    ``smugglersdrop.co.uk`` domain redirects to the canonical
    ``www.smugglersdrop.co.uk`` host, so we base everything on the www form to
    keep stored product URLs canonical.
    """

    def __init__(self, api_key: str | None = None):
        """Initialize Smuggler's Drop scraper."""
        super().__init__(
            roaster_name="Smuggler's Drop",
            base_url="https://www.smugglersdrop.co.uk",
    ...
    async def get_store_urls(self) -> list[str]:
        """Return the Squarespace shop (/shop "All") listing URL.

        The /shop page renders the full catalogue — all five whole-bean coffees
        plus the t-shirt / gift-card / subscription items that the exclusion
        patterns filter out downstream.
        """
        return ["https://www.smugglersdrop.co.uk/shop"]

    async def _extract_product_urls_from_store(self, store_url: str) -> list[str]:
        """Extract whole-bean coffee product URLs from the Squarespace listing page.

        Squarespace renders each product card with a ``.product-list-item`` class
        and a ``/shop/p/`` link (the card anchor is emitted with an unquoted
        ``href``, which BeautifulSoup parses into a normal attribute). We skip
        sold-out cards and then keep only coffee product URLs that are not
        excluded equipment/merch/subscriptions/gift cards.
        """
        ...
        product_urls = []
        for item in soup.select(".product-list-item, a[href*='/shop/p/']"):
            if item.name == "a":
    ...
    async def fetch_page(self, *args, **kwargs):
        ...
        # Smuggler's Drop's structured data lives in <head> meta tags. The
        # og:description carries name/tasting/origin/process; product:* carries
        # price, currency and availability. The visual/roast info comes from the
        # screenshot.
    ...
    def postprocess_extracted_bean(self, bean: CoffeeBean) -> CoffeeBean | None:
        """Hardcode GBP as the store currency (Smuggler's Drop emits product:price:currency)."""
        bean.currency = "GBP"
        return bean

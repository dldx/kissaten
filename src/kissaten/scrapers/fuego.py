"""Fuego Tostadores scraper implementation with AI-powered extraction."""
...
@register_scraper(
    name="fuego",
    display_name="Fuego Tostadores",
    roaster_name="Fuego Tostadores",
    website="https://fuegotostadores.com",
    description="Argentinian specialty coffee roaster offering single origin and blend coffees",
    requires_api_key=True,
    currency="ARS",
    country="Argentina",
    status="available",
)
class FuegoScraper(BaseScraper):
    """Scraper for Fuego Tostadores (fuegotostadores.com) with AI-powered extraction."""

    def __init__(self, api_key: str | None = None):
        """Initialize Fuego scraper.
    ...
    async def get_store_urls(self) -> list[str]:
        ...
        """
        return [
            "https://fuegotostadores.com/cafe-de-especialidad/cuartos-de-cafe/",
            "https://fuegotostadores.com/cafe-de-especialidad/cuartos-de-cafe/?mpage=2",
        ]
    ...
    def _minimize_soup_for_ai(self, soup: BeautifulSoup) -> None:
        ...
        Extracts product text (name, price, description, image URLs), structured
        data (JSON-LD, LS.variants), and key meta tags — discarding all CSS,
        JS, fonts, and layout chrome that bloats the page to ~570KB.
        """
        essentials = []
        ...
        sp = soup.find(id="single-product")
        if sp:
            spc = sp.find(id="single-product-container")
            if spc:
                left = spc.find(class_="js-product-left-col")
                right = spc.find(class_="js-product-right-col")

                text_parts = []

                if right:
                    np = right.find(class_="js-product-name-price-container")
                    if np:
                        text_parts.append(np.get_text(" ", strip=True))

                if left:
                    desc = left.find(class_="product-description-desktop")
                    if desc:
                        text_parts.append(desc.get_text(" ", strip=True))

                if right:
                    desc_mobile = right.find(class_="product-description-mobile")
                    if desc_mobile:
                        text_parts.append(desc_mobile.get_text(" ", strip=True))

                if left:
                    img_urls = []
                    for link in left.find_all("a", class_="js-product-slide-link"):
                        href = link.get("href")
                        if href:

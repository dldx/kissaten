"""Despiertoo scraper implementation with AI-powered extraction (Wix storefront)."""
...
@register_scraper(
    name="despiertoo",
    display_name="Despiertoo",
    roaster_name="Despiertoo",
    website="https://www.despiertoo.com",
    description=(
        "Specialty coffee roaster based in Granada, Spain, offering carefully "
        "sourced single-origin coffees with a focus on innovative fermentation "
        "processes."
    ),
...
class DespiertooScraper(BaseScraper):
    """Scraper for Despiertoo (despiertoo.com) — a Wix storefront.

    The product gallery lives on the homepage as Wix ``product-item-root``
    cards (with a ``data-slug`` attribute and ``product-page/`` links). Product
    detail pages embed all useful content inside ``div[data-hook="product-page"]``,
    which we narrow to before sending the HTML to the AI extractor.
    """

    # Non-coffee URL slug substrings (brewers, mugs, bottles).
    _excluded_url_slugs = [
        "asobu",  # brewer
        "fellow",  # Fellow carter mugs / accessories
        "miir",  # Miir bottles
        "botella",  # bottles
    ]

    def __init__(self, api_key: str | None = None):
        """Initialize Despiertoo scraper.
        ...
        super().__init__(
            roaster_name="Despiertoo",
            base_url="https://www.despiertoo.com",
    ...
    async def get_store_urls(self) -> list[str]:
        """Return the homepage URL, which hosts the Wix product gallery."""
        return ["https://www.despiertoo.com/"]
    ...
    # Sold-out Wix products carry an "Agotado"/"Agotada" ribbon or the marker
    # inside the card's aria-label (e.g. "Preventa Web Agotada" on the
    # Fermentation Project card). We skip any card whose text or aria-label
    # contains those markers before applying is_coffee_product_url filtering,
    # so excluded products don't leak past the stock check.
    async def _extract_product_urls_from_store(self, store_url: str) -> list[str]:
        """Extract product URLs from the Wix homepage gallery, filtering sold-out items.

        Args:
            store_url: URL of the listing page (homepage)
        ...
        for item in soup.select('[data-hook="product-item-root"]'):
            ...
            aria_label = (item.get("aria-label") or "").lower()
            if any(marker in item_text or marker in aria_label for marker in ("agotado", "agotada", "sold out")):
                logger.debug(f"Skipping sold-out product: {full_url}")
                continue

            # Drop brewer/mug/bottle slugs.

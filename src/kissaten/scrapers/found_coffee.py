"""Found Coffee scraper implementation with AI-powered extraction (Square Online)."""
...
@register_scraper(
    name="found-coffee",
    display_name="Found Coffee",
    roaster_name="Found Coffee",
    website="https://www.found.coffee",
    description="Australian specialty coffee shop and roastery in Toronto, Canada, "
...
class FoundCoffeeScraper(BaseScraper):
    """Scraper for Found Coffee (found.coffee) with AI-powered extraction.
    ...
    _STORE_API_BASE = "https://cdn5.editmysite.com/app/store/api/v28/editor/users/130530601/sites/933762021869263098"
    _LISTING_QUERY = "page={page}&per_page=100&sort_by=popularity_score&visibilities%5B%5D=visible"
    _max_listing_pages = 10

    # Non-coffee slugs to exclude: café drinks, food, merch, equipment, and
    # subscriptions. The Fermentation Project tasting kit is NOT excluded
    ...
    _excluded_url_slugs = [
        ...
        "pottery",
        "sticker",
        # Services / initiatives / subscriptions
        "subscription",
        "initiative",
        "found-forward",
        "after-hours",
    ...
    def __init__(self, api_key: str | None = None):
        """Initialize Found Coffee scraper.
        ...
        super().__init__(
            roaster_name="Found Coffee",
            base_url="https://www.found.coffee",
    ...
    async def _extract_product_urls_from_store(self, store_url: str) -> list[str]:
        ...
        while page <= min(total_pages, self._max_listing_pages):
            ...
            for item in items:
                ...
                # Exclude non-coffee products (drinks, food, merch, subscriptions)
                url_lower = url.lower()

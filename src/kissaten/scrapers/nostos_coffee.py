...
class NostosCoffeeScraper(BaseScraper):
    ...
    async def scrape(self, force_full_update: bool = False) -> list[CoffeeBean]:
        """Scrape coffee beans from Nostos Coffee with efficient stock updates.
    ...
    async def _extract_product_urls_from_store(self, store_url: str) -> list[str]:
        ...
        # Filter coffee products using both base class method and custom filtering
        coffee_urls = []
        for url in all_product_urls:
            # First use base class filtering
            if self.is_coffee_product_url(url, required_path_patterns=["/products/"]):
                # Then apply custom Nostos-specific filtering

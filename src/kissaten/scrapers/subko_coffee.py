...
@register_scraper(
    name="subko-coffee",
    display_name="Subko Coffee",
    roaster_name="Subko Coffee",
...
class SubkoCoffeeScraper(BaseScraper):
    """Scraper for Subko Coffee with AI-powered extraction."""

    def __init__(self, api_key: str | None = None):
        """Initialize Subko Coffee scraper.
    ...
    async def get_store_urls(self) -> list[str]:
        ...
        """
        return ["https://www.subko.coffee/pages/coffee"]
    ...
    async def _extract_product_urls_from_store(self, store_url: str) -> list[str]:
        ...
        product_grids = soup.select('.grid.grid--layout')
        if not product_grids:
            logger.warning(f"No product grid found on store page: {store_url}")
            return []
        all_product_url_el = product_grids[0].select('a.card__image[href*="/products/"]')
        all_product_urls = []
        ...
        excluded_patterns = ["pourtable-pourover"]
        coffee_urls = []

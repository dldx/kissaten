"""Copperopolis Coffee scraper implementation with AI-powered extraction.

Copperopolis Coffee is a Swansea (Wales, UK) specialty coffee roaster running
WordPress + WooCommerce. The site exposes the standard WooCommerce Store API at
...
Note: the storefront also lists merchandise (cupping spoon, beanie, t-shirt),
a cupping event and a "Roaster's Choice" subscription; only the coffee
products are kept.
"""
...
API_PRODUCTS_URL = "https://copperopolis.coffee/wp-json/wc/store/v1/products"


@register_scraper(
    name="copperopolis",
    display_name="Copperopolis Coffee",
    roaster_name="Copperopolis Coffee",
    website="https://copperopolis.coffee",
    description=(
        "Specialty coffee roaster based in Swansea, Wales, roasting "
        "single-origin coffees and hosting community cuppings."
    ),
...
class CopperopolisScraper(BaseScraper):
    """Scraper for Copperopolis Coffee (copperopolis.coffee) with AI extraction."""

    def __init__(self, api_key: str | None = None):
        """Initialize Copperopolis scraper.
        ...
        super().__init__(
            roaster_name="Copperopolis Coffee",
            base_url="https://copperopolis.coffee",
    ...
    async def _extract_product_urls_from_store(self, store_url: str) -> list[str]:
        ...
        page = 1
        while page <= 5:  # safety cap: 7 products today, 100 per page
        ...
        # Exclude merchandise (cupping spoon, beanie, t-shirt) and the
        # "Roaster's Choice" subscription. The Fermentation Project Cup-along
        # (fermentation-cup-along-swansea-slurp-02) is NOT excluded — it is
        # extracted like any other Fermentation Project product.
        excluded_slugs = [
            "cupping-spoon",
            "beanie",
            "t-shirt",
    ...
    def postprocess_extracted_bean(self, bean: CoffeeBean) -> CoffeeBean | None:
        """Force the currency to GBP as a final guard.
        ...
        falls back to GBP. Copperopolis sells in GBP.

        Args:
            bean: Extracted CoffeeBean object

        Returns:
            The bean with currency forced to GBP
        """
        bean.currency = "GBP"

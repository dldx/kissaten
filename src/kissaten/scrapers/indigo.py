"""Indigo Coffee Roasters scraper implementation with AI-powered extraction."""
...
@register_scraper(
    name="indigo",
    display_name="Indigo Coffee Roasters",
    roaster_name="Indigo Coffee Roasters",
    website="https://indigoroasters.com",
    description="Argentinian specialty coffee roaster offering blends, single origins, microlots and limited editions",
...
class IndigoScraper(BaseScraper):
    """Scraper for Indigo Coffee Roasters (indigoroasters.com) with AI-powered extraction."""

    def __init__(self, api_key: str | None = None):
        """Initialize Indigo scraper.
        ...
        super().__init__(
            roaster_name="Indigo Coffee Roasters",
            base_url="https://indigoroasters.com",
    ...
    async def get_store_urls(self) -> list[str]:
        ...
        """
        return [
            "https://indigoroasters.com/edicion-limitada1/",
            "https://indigoroasters.com/single-origins/",
            "https://indigoroasters.com/microlotes/",
            "https://indigoroasters.com/blends/",
        ]
    ...
    async def _extract_bean_with_ai(
        ...
        Replaces the full page HTML with a minimal soup containing only:
          - <title> and key <meta> tags (og:, twitter:, tiendanube:)
          - The <div id="single-product"> (name, description, price, variants)
          - Schema.org JSON-LD structured data
        This dramatically reduces tokens sent to the AI extractor.
        """
    ...
    def _minimize_soup_for_ai(self, soup: BeautifulSoup) -> None:
        ...
        Extracts product text (name, price, description, image URLs) and
        structured data (JSON-LD, LS.variants) — discarding CSS, JS, and
        layout chrome.
        """
        essentials = []
        ...
        for meta in soup.find_all("meta"):
            ...
            if any(k in prop for k in ["og:", "twitter:", "tiendanube:"]):
        ...
        pc = soup.find(class_="price-container")
        if pc:
            text_parts.append(pc.get_text(" ", strip=True))

        desc = soup.find(class_=lambda x: x and "product-description" in (x or ""))
        if desc:
            text_parts.append(desc.get_text(" ", strip=True))

        for col in soup.find_all(class_=lambda x: x and "product-image-column" in (x or "")):
            img_urls = []
            for link in col.find_all("a", class_="js-product-slide-link"):
                href = link.get("href")
                if href:
                    img_urls.append(str(href))
            if img_urls:
        ...
        for script in soup.find_all("script"):
            ...
            if "LS.variants" in text and "LS.currency" in text:
                essentials.append(script.extract())
                break
    ...
    async def _extract_product_urls_from_store(self, store_url: str) -> list[str]:
        ...
        # Sold-out detection: check for "Sin stock" text in the product container
        # Tiendanube marks out-of-stock products with a "Sin stock" badge.
        product_urls = []
        selectors = [
            'a[href*="/productos/"]',
            ".js-item-product a",
        ...
        for selector in selectors:
            ...
            if links:
                for link in links:
                    ...
                    # Sold-out detection: check for "Sin stock" text in parent container
                    ...
                    if product_block and isinstance(product_block, Tag):
                        ...
                        if "sin stock" in block_text.lower():
    ...
    def _get_excluded_url_patterns(self) -> list[str]:
        """Extend excluded URL patterns to skip 1kg variants.

        1kg bags are separate products that duplicate the 250g listings;
        we only want the smaller format.

        Returns:
            List of URL patterns to exclude
        """
        return super()._get_excluded_url_patterns() + ["-1kg", "-1-kg"]

"""Lantern Coffee scraper implementation with AI-powered extraction (WooCommerce).

Lantern Coffee Bar and Lounge (Grand Rapids, Michigan) runs a WooCommerce
storefront (WordPress + Elementor, `lanterncoffee.com`). The product detail
pages are server-rendered HTML, but the WooCommerce Store API
(``/wp-json/wc/store/v1/products``) exposes complete structured product data
(name, full description with origin/process/varietal/elevation/tasting notes,
price range in minor units, weight × grind-size variations, stock status,
categories) — so the scraper uses the Store API for both listing and detail
instead of parsing HTML:

- Listing: ``.../products?per_page=100&page=N`` (name, permalink, categories,
  ``is_in_stock``) — coffee products carry the ``Coffee`` category, while
  merch and subscriptions carry ``Merch``/``Subscriptions``.
- Detail: ``.../products?slug=<slug>`` returns the same product shape, from
  which a compact soup is built and fed to the AI extractor (driftwood.py
  pattern). This avoids one heavyweight (300+ KB) HTML fetch per product and
  keeps the origin/process/tasting-note text that the Elementor page buries
  in styled blocks.
...
@register_scraper(
    name="lantern-coffee",
    display_name="Lantern Coffee",
    roaster_name="Lantern Coffee",
    website="https://lanterncoffee.com",
    description="Coffee bar and lounge in Grand Rapids, Michigan, roasting single origins and "
    "blends with a praised subscription newsletter.",
    requires_api_key=True,
    currency="USD",
    country="United States",
    status="experimental",
)
class LanternCoffeeScraper(BaseScraper):
    """Scraper for Lantern Coffee (lanterncoffee.com) using the WooCommerce Store API."""

    # WooCommerce Store API endpoint (public, no auth required).
    _STORE_API_URL = "https://lanterncoffee.com/wp-json/wc/store/v1/products"
    _LISTING_QUERY = "per_page=100&page={page}"
    _max_listing_pages = 10

    # Merch is all in the "Merch" category, subscriptions in "Subscriptions",
    # so category filtering does the heavy lifting. These URL-slug patterns are
    # a belt-and-braces guard. Sampler/taster packs are NOT excluded — they are
    # flagged is_tasting_kit/requires_review downstream.
    _excluded_url_slugs = [
        "subscription",
        "hoodie",
        "t-shirt",
    ...
    def __init__(self, api_key: str | None = None):
        """Initialize Lantern Coffee scraper.
        ...
        super().__init__(
            roaster_name="Lantern Coffee",
            base_url="https://lanterncoffee.com",
    ...
    def _build_product_soup(cls, product: dict) -> BeautifulSoup:
        ...
        # Variations carry Weight × Grind Size attribute pairs; collapse them
        # into the unique values per attribute so the AI extractor sees the
        # available sizes without 12+ near-duplicate lines.
    ...
    async def _fetch_store_product_soup(self, product_url: str) -> BeautifulSoup | None:
        ...
        Args:
            product_url: Product detail URL (``https://lanterncoffee.com/product/<slug>/``)
    ...
    async def _extract_product_urls_from_store(self, store_url: str) -> list[str]:
        ...
        for page in range(1, self._max_listing_pages + 1):
            ...
            for item in items:
                ...
                # Only the "Coffee" category is coffee; merch/subscriptions
                # live in their own categories.

...
@register_scraper(
    name="aery-coffee",
    display_name="Aery Coffee",
    roaster_name="Aery Coffee",
    website="https://aerycoffee.com",
    description="Specialty coffee roaster based in South Korea",
    requires_api_key=True,
    currency="KRW",
    country="South Korea",
    status="available",
)
class AeryCoffeeScraper(BaseScraper):
    """Scraper for Aery Coffee with AI-powered extraction."""

    def __init__(self, api_key: str | None = None):
        """Initialize Aery Coffee scraper.
    ...
    async def _extract_product_urls_from_store(self, store_url: str) -> list[str]:
        ...
        all_product_url_el = soup.select('a[href*="/product/"][spec="prd_brand"]')
        all_product_urls = []
        ...
        for url in all_product_urls:
            if self.is_coffee_product_url(url, required_path_patterns=["/product/"]) and not any(
                ...
                coffee_urls.append(f"https://aerycoffee.com{url}")

"""Coopers Coffee scraper implementation with AI extraction.

Coopers Coffee (cooperstradingcompany.com) is a Squarespace-hosted
storefront (recovered from the dead cooperscoffee.co.uk). Products live at
``/shop/p/...`` and carry Squarespace product-page meta tags (``og:title``,
...
Discovery crawls the ``/shop`` listing and keeps whole-bean coffee products
only — excluding equipment (Hario cold-brew makers, Fellow Atmos jars, Keep
Cups), classes, and merchandise (hoodies/sweatshirts/tshirts). Curated
samplers (e.g. "The Big 3!") are retained and flagged for review.
"""
...
@register_scraper(
    name="coopers",
    display_name="Coopers Coffee",
    roaster_name="Coopers Coffee",
    website="https://cooperstradingcompany.com",
    description=(
        "Specialty coffee roaster based in Marlow, UK, offering single-origin "
        "and blended whole-bean coffees roasted fresh from a micro roastery."
    ),
...
class CoopersCoffeeScraper(BaseScraper):
    """Scraper for Coopers Coffee (cooperstradingcompany.com) — a Squarespace storefront.
    ...
    def __init__(self, api_key: str | None = None):
        """Initialize Coopers Coffee scraper."""
        super().__init__(
            roaster_name="Coopers Coffee",
            base_url="https://cooperstradingcompany.com",
    ...
    async def get_store_urls(self) -> list[str]:
        """Return the Squarespace shop (/shop) listing URL."""
        return ["https://cooperstradingcompany.com/shop"]

    def _get_excluded_url_patterns(self) -> list[str]:
        """Add Coopers-specific non-coffee exclusions on top of the base set.

        The base list already drops generic equipment/merch/service patterns
        (hario, v60, grinder, subscription, gift-card, shirt, ...). Coopers
        needs a few more: brewing classes, hot chocolate, keep cups, and green
        (unroasted) coffee.
        """
        return super()._get_excluded_url_patterns() + [
            "class",  # "introduction-to-coffee-class", "latte-art-class-2-hours"
            "hotchocolate",
            "chocolate",
            "keep-cup",
            "keepcup",
            "atmos",  # Fellow Atmos coffee jar (sold under a keep-cup handle)
            "tshirt",
            "sweatshirt",
            "greencoffee",  # unroasted green coffee — not a roasted whole-bean product
            "green-coffee",
        ]

    def _get_tasting_kit_url_patterns(self) -> list[str]:
        """Add the Coopers sampler handle to the base kit/taster patterns."""
    ...
    async def _extract_product_urls_from_store(self, store_url: str) -> list[str]:
        ...
        and an ``/shop/p/`` link. We skip sold-out cards and then keep only
        coffee product URLs that are not excluded equipment/merch/classes.
        """
    ...
    async def fetch_page(self, *args, **kwargs):
        ...
        # Coopers' structured data lives in <head> meta tags. The og:description
    ...
    def postprocess_review_flags(self, bean, url: str):
        """Flag Coopers' curated sampler ("The Big 3!") as a tasting kit.

        The handle ``javanatural-jcdfm`` carries no kit token, but the product is a
        multi-origin tasting box, so we force ``is_tasting_kit`` here so it lands in
        the admin review queue instead of public search.
        """
        name = getattr(bean, "name", "") or ""
        if "big 3" in str(name).lower() or "big-3" in str(url).lower():
            bean.is_tasting_kit = True
        return bean

    def postprocess_extracted_bean(self, bean: CoffeeBean) -> CoffeeBean | None:
        """Hardcode GBP as the store currency (Coopers emits product:price:currency)."""
        bean.currency = "GBP"
        return bean
